...
hesitation_col = [col for col in df.columns if "Hesitant" in col]
categories = ["Robot 1", "Robot 2", "Both appear equally hesitant", "Both appears equally hesitant", "Neither appears hesitant"]
counts = {}
for col in hesitation_col:
...
counts_df = (pd.DataFrame(counts)
               ...
               .astype(int))
if "Both appears equally hesitant" in counts_df.index:
    counts_df.loc["Both appear equally hesitant"] = (
        counts_df.loc["Both appear equally hesitant"].fillna(0)
        + counts_df.loc["Both appears equally hesitant"].fillna(0)
    )
    counts_df = counts_df.drop(index="Both appears equally hesitant")
print(counts_df)
...
expression_labels = [r"Pause Duration ($\tau_{pause}$)", r"Retreat Distance ($d_{retreat}$)", r"Tilt Angle ($\chi_{tilt}$)", r"Retreat Count ($n_{retreat}$)", r"Approach Acceleration ($\alpha_{approach}$)", r"Retreat Acceleration ($\alpha_{retreat}$)"]
fig, axes = plt.subplots(3, 2, figsize=(10, 8))
axes = axes.flatten()
#short_labels = ["Robot 1", "Robot 2", "Same", "Neither"]
# Loop through each expression and corresponding subplot
