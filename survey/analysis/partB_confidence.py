...
confidence_col = [col for col in df.columns if "Confidence" in col]
#confidence_col = ["Confidence: DOP", "Confidence SOA"]
categories = ["Robot 1", "Robot 2", "Both appear equally confident", "Neither appears confident"]
counts = {}
for col in confidence_col:
...
axes = axes.flatten() 
expression_labels= [r"Pause Duration ($\tau_{pause}$)", r"Approach Acceleration ($\alpha_{approach}$)"]
...
for i, expr in enumerate(counts_df.columns):
    ...
    colors = ["red" if cat == predicted_labels[expr] 
              else "skyblue" 
