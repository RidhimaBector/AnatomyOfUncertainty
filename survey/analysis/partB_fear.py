...
fear_col = [col for col in df.columns if "Fear" in col]
#fear_col = ["Fear DOP\xa0", "Fear DOS", "Fear_NOP", "Fear_NOR", "Fear_SOA", "Fear_SOR"]
categories = ["Robot 1", "Robot 2", "Both appear equally fearful", "Neither appears fearful"]
counts = {}
for col in fear_col:
...
expression_labels = [r"Pause Duration ($\tau_{pause}$)", r"Shiver Amplitude ($\chi_{shiver_{j}}$)", r"Pause Count ($n_{pause}$)", r"Retreat Count ($n_{retreat}$)", r"Approach Acceleration ($\alpha_{approach}$)", r"Retreat Acceleration ($\alpha_{retreat}$)"]
